import { ChatBubbleLeftRightIcon, ClockIcon, ArrowTrendingUpIcon, ChartBarIcon } from "@heroicons/react/24/outline";
  ...
  const features = [
    {
      ...
      desc: "ChatBot247 sử dụng mô hình GPT-Plus được huấn luyện chuyên sâu dựa trên dữ liệu và kinh nghiệm bán hàng thực tế.",
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-10">
          {features.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className="relative bg-white rounded-2xl p-5 sm:p-8 shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-2 flex flex-col"
              >
                <div className="absolute -top-3 sm:-top-4 left-4 sm:left-6 bg-orange-500 text-white text-xs sm:text-sm font-bold px-3 sm:px-4 py-1 sm:py-1.5 rounded-full shadow-lg">
                  {item.id}
                </div>

                <div className="w-11 h-11 sm:w-14 sm:h-14 flex items-center justify-center rounded-xl bg-orange-100 text-orange-600 mb-4 sm:mb-6 mt-2 sm:mt-0">
                  <Icon className="w-5 h-5 sm:w-7 sm:h-7 stroke-2" />
                </div>

                <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-2 sm:mb-4 min-h-[48px] sm:min-h-[56px]">
                  {item.title}
                </h3>
